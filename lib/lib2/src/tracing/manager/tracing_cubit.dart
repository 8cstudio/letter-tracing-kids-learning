...
import '../model/letter_paths_model.dart';
import '../model/trace_geometric_shape_model.dart';
import '../model/trace_model.dart';
import '../model/trace_shape_model.dart';
import '../model/trace_word_model.dart';
...
class TracingCubit extends Cubit<TracingState> {
        ...
        traceLetter: TypeExtensionTracking().getTracingData(
          geometryShapes:
              state.stateOfTracing == StateOfTracing.traceShapes &&
                      state.traceGeoMetricShapes!.isNotEmpty
                  ? state.traceGeoMetricShapes![state.index].shapes
                  : null,
          chars:
              state.stateOfTracing == StateOfTracing.chars &&
  ...
  Future<void> loadAssets() async {
    ...
    for (var e in state.traceLetter) {
      ...
      final transformedPath = _applyTransformation(parsedPath, viewSize);
  ...
  Future<List<List<Offset>>> _loadPointsFromJson(
    ...
    for (var stroke in jsonData['strokes']) {
      ...
      final points =
          strokePointsData.map<Offset>((pointString) {
            final coords =
                pointString.split(',').map((e) => double.parse(e)).toList();
            return Offset(
              coords[0] * viewSize.width,
              coords[1] * viewSize.height,
            );
          }).toList();
  ...
  void handlePanStart(Offset position) {
    ...
    final currentStrokePoints =
        state.letterPathsModels[state.activeIndex].allStrokePoints[state
    ...
    if (state.letterPathsModels[state.activeIndex].currentStrokeProgress >= 0 &&
      ...
      if (currentStrokePoints.length == 1) {
        ...
        if (isValidPoint(
          ...
          final newDrawingPath =
              Path()
                ..moveTo(singlePoint.dx, singlePoint.dy)
                ..lineTo(
                  currentStrokePoints.first.dx,
                  currentStrokePoints.first.dy,
                );
  ...
  void completeStroke() {
    ...
    if (currentStrokeIndex < currentModel.allStrokePoints.length - 1) {
      ...
      final endPointOfPreviousStroke =
          previousStrokePoints.isNotEmpty
              ? currentModel
