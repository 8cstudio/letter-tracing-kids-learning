...
import '../colors/phonetics_color.dart';
import '../enums/shape_enums.dart';
import '../phontics_constants/arabic_shape_paths_blue_unit.dart';
import '../phontics_constants/arabis_shape_paths.dart';
import '../phontics_constants/english_shape_path2.dart';
import '../phontics_constants/math_trace_shape_paths.dart';
import '../phontics_constants/numbers_svg.dart';
import '../phontics_constants/shape_paths.dart';
import '../points_manager/shape_points_manger.dart';
import '../tracing/model/trace_geometric_shape_model.dart';
import '../tracing/model/trace_model.dart';
import '../tracing/model/trace_shape_model.dart';
import '../tracing/model/trace_word_model.dart';
...
class TypeExtensionTracking {
  ...
  List<TraceModel> getTracingData({
    ...
    if (currentOfTracking == StateOfTracing.traceShapes) {
      tracingDataList.addAll(
        getListOfTracingDataMathShapes(shapes: geometryShapes!),
      );
      ...
      for (var char in chars) {
        ...
        if (_isArabicCharacter(letters)) {
          tracingDataList.addAll(
            _getTracingDataArabic(letter: letters).map(
              (e) => e.copyWith(
          ...
          tracingDataList.add(
            _getTracingDataNumbers(number: letters).first.copyWith(
              innerPaintColor: char.traceShapeOptions.innerPaintColor,
              outerPaintColor: char.traceShapeOptions.outerPaintColor,
              indexColor: char.traceShapeOptions.indexColor,
              dottedColor: char.traceShapeOptions.dottedColor,
          ...
          final uppers = _getTracingDataPhonicsUp(
            letter: letters.toLowerCase(),
          );
          final newBigSizedUppers =
              uppers
                  .map(
                    (up) => up.copyWith(letterViewSize: const Size(300, 300)),
                  )
                  .toList();
          tracingDataList.add(
            newBigSizedUppers.first.copyWith(
              innerPaintColor: char.traceShapeOptions.innerPaintColor,
              outerPaintColor: char.traceShapeOptions.outerPaintColor,
              indexColor: char.traceShapeOptions.indexColor,
              dottedColor: char.traceShapeOptions.dottedColor,
  ...
  List<TraceModel> getListOfTracingDataMathShapes({
    ...
    for (var sh in shapes) {
      traceModels.add(
        getTracingDataMathShapes(currentLetter: sh.shape).first.copyWith(
  ...
  List<TraceModel> getTracingDataMathShapes({
    ...
    switch (currentLetter) {
      case MathShapes.circle:
        ...
        return [circ.copyWith(letterViewSize: const Size(200, 200))];
      ...
      case MathShapes.rectangle:
        ...
        return [rect.copyWith(letterViewSize: const Size(160, 160))];
    ...
    while (i < word.length) {
      if (word[i] == '1' && i + 1 < word.length && word[i + 1] == '0') {
        ...
        if (word[i] == '1' && word[i + 1] == '0') {
          letters.add(
            _getTracingDataNumbers(number: '10').first.copyWith(
              isSpace:
                  (i + 2 < word.length &&
      ...
      if (_isArabicCharacter(word[i])) {
        letters.add(
          _getTracingDataArabic(
            letter: word[i],
          ).first.copyWith(isSpace: isNextSpace),
        );
      } else if (_isNumber(word[i])) {
        letters.add(
          _getTracingDataNumbers(
            number: word[i],
          ).first.copyWith(isSpace: isNextSpace),
        );
  ...
  List<TraceModel> _getTracingDataPhonics({
    ...
    PhonicsLetters currentLetter = _detectTheCurrentEnumFromPhonics(
      letter: letter,
    );
    ...
    switch (currentLetter) {
      ...
      case PhonicsLetters.o:
        return [
          _getTracingDataPhonics(
            letter: 'o',
            sizeOfLetter: const Size(200, 200),
          ).first,
        ];
      ...
      case PhonicsLetters.c:
        return [_getTracingDataPhonics(letter: 'c').first];
      case PhonicsLetters.r:
        return [
          TraceModel(
